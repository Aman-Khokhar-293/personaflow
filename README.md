...
- **TTS (Avatar Voice)**: Migrated from `edge-tts` (Microsoft) to **Kokoro TTS** (local neural TTS). The `/api/avatar/talk` endpoint now uses `KPipeline` from the `kokoro` Python package. Outputs `.wav` audio. On first run, Kokoro auto-downloads its model weights (~82MB). No API key required.
